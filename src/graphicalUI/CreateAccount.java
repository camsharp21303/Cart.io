...
public class CreateAccount {
	...
	private static Label fileName, errorL;
	...
	public static void activateCreateAccount() {
		...
		box = new VBox(logo, title, nameTF, emailTF, phoneTF, usernameTF, addressTF, passwordTF, confirmPassTF,
				imagePick, create);
	...
	private static void createUser() {
		...
		if (nameTF.getText().isEmpty() || usernameTF.getText().isEmpty() || addressTF.getText().isEmpty()
				|| passwordTF.getText().isEmpty()) {
			showErrorText("Required Information Missing");
			return;
		}
		...
		Customer customer = new Customer(firstName, lastName, usernameTF.getText(), phoneTF.getText(),
				emailTF.getText(), addressTF.getText(), null);

		customer.commit(passwordTF.getText(), imageFile);

		Main.login(SQLcommands.login(customer.getUsername(), passwordTF.getText()));
	}

	private static void showErrorText(String error) {
		errorL.setText(error);
		if (!box.getChildren().contains(errorL)) {
			box.getChildren().add(2, errorL);
		}
	}
}
